...
def init_word():
    ...
    total, word_size = f.readline().strip().split()[:2]
    total = (int)(total)
...
def init_train_files(name):
    ...
    return np.array(instance_triple), np.array(
        instance_scope), sen_len, sen_label, sen_word, sen_pos1, sen_pos2, sen_mask
...
def init_test_files(name):
    ...
    for s in range(total):
        ...
        if instance_triple_with_NA == [] or instance_triple_with_NA[len(instance_triple_with_NA) - 1] != tup:
            if instance_triple_with_NA == [] or instance_triple_with_NA[len(instance_triple_with_NA) - 1][:2] != tup[
                                                                                                                 :2]:
                instance_scope.append([s, s])
        ...
        if (s + 1) % 100 == 0:
            print(s)
    return np.array(instance_entity), np.array(instance_entity_no_bag), np.array(instance_triple), np.array(
        instance_scope), sen_len, sen_label, sen_word, sen_pos1, sen_pos2, sen_mask
...
instance_triple, instance_scope, train_len, train_label, train_word, train_pos1, train_pos2, train_mask = init_train_files(
    "train_sort")
...
instance_entity, instance_entity_no_bag, instance_triple, instance_scope, test_len, test_label, test_word, test_pos1, test_pos2, test_mask = init_test_files(
    "test_sort")
