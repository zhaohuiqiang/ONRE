...
class Encoder(object):
    ...
    def rnn(self, x, hidden_size, sequence_length, cell_name='lstm'):
        with tf.name_scope('rnn'):
            ...
            _, states = tf.nn.dynamic_rnn(cell, x, sequence_length=sequence_length, dtype=tf.float32,
                                          scope='dynamic-rnn')
    ...
    def birnn(self, x, hidden_size, sequence_length, cell_name='lstm'):
        with tf.name_scope('bi-rnn'):
            ...
            _, states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, x, sequence_length=sequence_length,
                                                        dtype=tf.float32, scope='dynamic-bi-rnn')
