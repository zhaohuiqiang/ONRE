...
def cnn_max_adv(is_training):
    if is_training:
        with tf.variable_scope('cnn_max_adv', reuse=False):
            ...
            x = framework.encoder.cnn(embedding, FLAGS.hidden_size, activation=tf.nn.relu)
        ...
        with tf.variable_scope('cnn_max_adv', reuse=True):
            x = framework.encoder.cnn(new_embedding, FLAGS.hidden_size, activation=tf.nn.relu)
            logit, repre = framework.selector.maximum(x, framework.scope, framework.label_for_select)
            loss = framework.classifier.softmax_cross_entropy(logit)
